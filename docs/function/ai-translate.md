...
由于在线翻译服务价格不同，各个翻译服务每个月也将有**各自独立且不同的使用额度**。使用 GPT 或其他大语言模型服务时，将按翻译的字符（token）数量计算额度。使用 Azure 翻译服务时，每一次翻译字幕的操作将消耗 1 次使用额度。

当前弹弹play会员用户的月额度上限是：（最后更新时间：2026/2/1，文档可能会更新不及时，请以软件中提示为准）

- GPT-5：100,000 token/月
- GPT-5 mini: 300,000 token/月
...
举例来说，如果您于1月20日充值了一个月会员开始使用翻译服务，那么您可以在1月20日到1月31日之间使用完整的1月份额度。到了2月1日，您的剩余额度将被重置，还可以从0开始继续享受下一个月的整月额度，但1月份未用完的剩余额度不会保留至2月。
