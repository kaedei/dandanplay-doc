...
![UptimeRobot](/images/upload/online_1.png)
[弹弹play服务状态页](https://stats.uptimerobot.com/DV0BKUo2g9) |
<img src="https://wiki.connect.qq.com/wp-content/uploads/2021/01/bt_blue_24X24.png" width="24px">[开发者交流群(QQ)](https://qm.qq.com/q/D2nRsB3mCY) |
...
弹弹play开放弹幕网络，就是基于弹弹play播放器的**弹幕收发、文件识别**功能发展出来的数据开放共享的网络服务。目的是为了重新组织互联网上复杂混乱的视频文件世界，将它们识别出来并进行归类。
...
开放弹幕网络提供基于弹幕功能的基础设施服务，方便开发者在自己的应用中集成弹幕和其他必要的功能。当前主要提供以下服务：

- **弹幕收发**：弹幕收发是弹弹play开放弹幕网络的核心功能，开发者可以在自己的应用中集成弹幕功能，使用弹弹play的弹幕库来提供丰富的弹幕体验。
- **文件识别**：使用文件hash和文件名来识别视频文件，获取对应的节目编号（`episodeId`），从而关联到正确的弹幕库。
- **搜索**：通过搜索 API，开发者可以在自己的应用中集成搜索功能，帮助用户查找番剧和节目。
- **番剧信息**：获取番剧和节目的详细信息，包括标题、简介、封面图、关联的第三方网站等。
- **首页推荐**：获取首页推荐的新番列表、公告、热门番剧等信息，帮助用户了解最新的内容。
- **排行榜**：基于弹弹play开放弹幕网络的真实数据，提供热门番剧、最近飙升等排行榜信息，帮助用户发现受欢迎的内容。
...
本指南旨在帮助第三方开发者正确地集成和使用弹弹play开放弹幕网络服务。弹弹play开放弹幕网络 API v2支持两种身份验证模式：签名验证模式和凭证模式。以下内容将详细介绍如何配置和使用这两种模式。
...
弹弹play开放弹幕网络 API 支持使用 **签名验证模式** 或 **凭证模式** 两种身份验证方式，您可以选择其中一种来验证您的请求。

#### 请求头参数

所有发往本服务的请求必须在 HTTP 请求头中包含以下信息：
...
- 如果您的应用程序是一个服务器端应用，或者您有能力保护 `AppSecret`，那么可以使用 **凭证模式**。

:::

### 5. 签名验证模式指南

若您选用**签名验证模式**，请按照以下步骤生成请求头中的`X-Signature`值
...
- **403 Forbidden**: 
  ...
  | `Invalid AppId` | 签名验证模式下 `AppId` 或 `AppSecret` 无效。凭证模式下 `AppId` 无效。 |
  | `Invalid Signature` | 签名不匹配。 |
  | `Invalid AppSecret` | 凭证模式下 `AppSecret` 无效。 |
...
- 如果您正在开发开源的客户端、前端项目，可以选择：
  - 通过自建服务器端向弹弹play开放弹幕网络转发来自客户端的请求，将 `AppSecret` 存放在您的服务器端。
...
我们鼓励您结合已有应用，使用弹弹play开放弹幕网络 API 来提供更好的用户体验。例如：

- 在您的应用中集成弹幕功能，使用弹弹play的弹幕库来提供丰富的弹幕体验
- 使用文件识别 API 来帮助用户快速匹配视频文件和番剧节目
- 使用搜索 API 来帮助用户查找番剧和节目

#### (3) 我们禁止：

- 禁止通过 API 进行恶意行为：
  - 禁止发送大量无意义或恶意的弹幕污染弹幕库环境
  - 禁止向服务器大量提交无意义的数据
- 禁止滥用 API：
  - 禁止通过 API 进行规模化抓取数据等“批量下载弹幕”、“下载数据库”的行为
  - 服务器算力、带宽和流量都是宝贵且昂贵的资源。请合理使用 API，避免对服务器造成过大的负担，影响其他用户和开发者的正常使用。重新强调：**请结合用户的实际操作调用 API，并按需使用**
- 禁止将弹弹play开放弹幕网络的 API 用于商业目的，除非获得弹弹play方面的明确授权

::: tip 关于商用行为

未经授权，禁止利用弹弹play开放弹幕网络返回的数据进行任何形式的商业活动，包括但不限于向第三方收费或提供增值服务。如需商业合作或授权，请与我们联系。

在未经授权的情况下，若您的应用接入了弹弹play开放弹幕网络作为主要弹幕来源，那么您不能使用弹弹play开放弹幕网络的功能和数据作为卖点向用户或第四方收费。举例来说：
...
当前不限制 API 的调用频率，但如果我们发现您的应用程序有违反上述《使用约定》的行为，我们保留在不事先通知的情况下立即停用您应用的权利。

为保障弹弹play开放弹幕网络服务长期稳定运行，**从2026年6月25日**起，已启用应用分层与额度管理机制。不同项目将根据实际情况获得对应的接入层级和支持策略。[查看详情](https://dev.dandanplay.com/PublicPage/Quota)

为了保证弹弹play开放弹幕网络的正常服务，我们对部分比较消耗服务器资源和带宽流量的接口（如搜索、获取弹幕）配置了检测机制，如果检测到您的应用远超正常范围内地调用这些接口，程序将自动限制您的继续访问。如果您的应用确实需要频繁调用这些接口，请联系我们添加白名单。

### 10. 缓存建议

即使是最简单的缓存策略也能够带来性能和响应速度的大幅提升。建议您在调用弹弹play开放弹幕网络 API 时，适当缓存 API 返回的数据，减少请求次数。以下是一些缓存建议：
...
如果您的应用调用量非常大或非常频繁，建议从一开始接入时就考虑使用缓存机制，以免触发检测与拦截机制。
...
- 测试视频：下载地址：[https://kaedei.lanzouo.com/itjCq30geele](https://kaedei.lanzouo.com/itjCq30geele)。此视频前16MB数据的MD5为 `658d05841b9476ccc7420b3f0bb21c3b`
...
当用户想要发送弹幕时，可以再次调用 **应用弹幕 API**（`/api/v2/comment/{episodeId}/app`）将弹幕发送到弹弹play服务器上。
